...
from services.models.education import EducationNewService, EducationNewServiceImage
...
# Vendor: List + Create
# -----------------------------------------------
class EducationServiceListCreateView(APIView):
    ...
    def get(self, request):
        ...
        serializer = EducationNewServiceSerializer(
    ...
    def post(self, request):
        ...
        service = EducationNewService.objects.create(
            vendor=vendor,
        ...
        for img_file in request.FILES.getlist('multi_images'):
            img_instance = EducationNewServiceImage.objects.create(image=img_file)
            service.multi_images.add(img_instance)

        serializer = EducationNewServiceSerializer(service, context={'request': request})
...
# -----------------------------------------------
class EducationServiceDetailView(APIView):
    permission_classes = [IsAuthenticated]
    parser_classes = [MultiPartParser, FormParser]

    def get_object(self, pk, vendor):
        return get_object_or_404(EducationNewService, pk=pk, vendor=vendor)
    ...
    def put(self, request, pk):
        ...
        if request.FILES.getlist("multi_images"):
            for img in request.FILES.getlist("multi_images"):
                img_instance = EducationNewServiceImage.objects.create(image=img)
                service.multi_images.add(img_instance)
...
class PublicEducationListView(generics.ListAPIView):
    ...
    serializer_class = EducationNewServiceSerializer

    def get_queryset(self):
        qs = EducationNewService.objects.filter(status='approved', subcategory__status='Active')
...
# -----------------------------------------------
class PublicEducationDetailView(APIView):
    permission_classes = [AllowAny]

    def get(self, request, id):
        try:
            service = EducationNewService.objects.get(id=id, status='approved', subcategory__status='Active')
            serializer = EducationNewServiceSerializer(service, context={'request': request})
            return Response(serializer.data)
        except EducationNewService.DoesNotExist:
...
# Admin: Approval
# -----------------------------------------------
class EducationServiceApprovalAPIView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def patch(self, request, pk):
        ...
        service = get_object_or_404(EducationNewService, pk=pk)
        ...
        return Response({
            "message": "Education service status updated successfully",
