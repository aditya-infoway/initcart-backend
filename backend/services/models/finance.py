...
class FinanceService(ServiceBaseModel):
    STATUS_CHOICES = (
        ('pending', 'Pending'),
        ('approved', 'Approved'),
        ('rejected', 'Rejected'),
    ...
    vendor = models.ForeignKey(Vendor, on_delete=models.CASCADE, related_name="finance_services")
    ...
    # Images
    main_image = models.ImageField(upload_to='finance_services/', null=True, blank=True)
    multi_images = models.ManyToManyField('FinanceServiceImage', blank=True)
    ...
    approved_by = models.ForeignKey(
        ...
        related_name='approved_finance_services'
    )
...
class FinanceServiceImage(models.Model):
