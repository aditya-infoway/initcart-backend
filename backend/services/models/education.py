...
class EducationNewService(ServiceBaseModel):
    STATUS_CHOICES = (
        ('pending', 'Pending'),
        ('approved', 'Approved'),
        ('rejected', 'Rejected'),
    ...
    vendor = models.ForeignKey(Vendor, on_delete=models.CASCADE, related_name="education_new_services")
    subcategory = models.ForeignKey(ServiceSubcategory, on_delete=models.SET_NULL, null=True, blank=True)

    # Basic Info
    business_name = models.CharField(max_length=255)
    address = models.TextField()
    location = models.CharField(max_length=500, help_text="Google Maps link", blank=True, null=True)
    ...
    # Images
    main_image = models.ImageField(upload_to='education_new_services/', null=True, blank=True)
    multi_images = models.ManyToManyField('EducationNewServiceImage', blank=True)
    ...
    class Meta:
        db_table = 'education_new_service'  # New unique table name
        ordering = ['-created_at']
...
class EducationNewServiceImage(models.Model):
