...
class TravelAgencyService(ServiceBaseModel):
    ...
    main_image = models.ImageField(upload_to='TravelAgency_services/', null=True, blank=True)
    second_image = models.ImageField(upload_to='TravelAgency_services/', null=True, blank=True)
    multi_images = models.ManyToManyField('TravelAgencyImage', blank=True)
...
class TravelAgencyItem(models.Model):
    service = models.ForeignKey(TravelAgencyService, on_delete=models.CASCADE, related_name='items')
